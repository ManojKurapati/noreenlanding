    ...
    <section id="services" className="py-24 bg-slate-950/60 relative border-t border-slate-900">
          ...
          {servicesData.map((service) => {
            ...
            return (
              <div
                ...
                className="bg-slate-900/30 border border-slate-900 rounded-2xl p-6 lg:p-8 hover:border-slate-800 hover:bg-slate-900/50 transition-all duration-300 group cursor-pointer flex flex-col justify-between"
              >
                <div>
                  {/* Icon */}
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-tr ${service.accent} text-slate-950 mb-6 shadow-md`}>
                    <IconComp className="w-6 h-6" />
                  </div>
                  
                  {/* Title & Tagline */}
                  <h3 className="text-xl font-bold text-slate-200 mb-2 group-hover:text-slate-100 transition-colors">
                    {service.title}
                  </h3>
      ...
      {selectedService && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          {/* Drawer content */}
          <div className="relative w-full max-w-lg bg-slate-900 border-l border-slate-800 shadow-2xl h-full flex flex-col justify-between z-10 animate-in slide-in-from-right duration-300">
            <div>
              {/* Header */}
              <div className="p-6 border-b border-slate-800 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-100">{selectedService.title}</h3>
              ...
              <div className="p-6 overflow-y-auto space-y-6 max-h-[calc(100vh-200px)]">
                <div className="space-y-2">
                  <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Overview</h4>
                  <p className="text-slate-300 text-sm leading-relaxed">{selectedService.description}</p>
                </div>
                
                <div className="space-y-4">
                  <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Specific Capabilities</h4>
                  <ul className="space-y-3">
                    {selectedService.capabilities.map((cap, i) => (
                      <li key={i} className="flex items-start gap-3 bg-slate-950/40 border border-slate-800/40 rounded-xl p-3.5">
